...
- [~] **C04 · Completar:** giro de cabeza mediante vistas dibujadas frontal/¾/perfil; coordinar sustituciones, orden y correctivos. No presentar un giro 360 automático si faltan vistas. — **parcial** en v4.44.0+: juego de vistas en el modelo (`rigViewSet`: elección discreta por valor de control, orden de slots por vista, `rigViewSetCoverage` que dice qué está dibujado y qué falta) y panel «Giro por vistas» que muestra la cobertura en vez de prometer la vuelta. 28 pruebas (`run_view_sets_tests`) y `check_giro_vistas_ui`, ocho mutaciones verificadas. **Falta:** coordinar los CORRECTIVOS por vista —hoy las acciones se conducen por el mismo control y se suman, pero no hay corrección propia de cada vista— y la revisión artística con un personaje dibujado de verdad, que es lo único que dice si el giro se lee.
